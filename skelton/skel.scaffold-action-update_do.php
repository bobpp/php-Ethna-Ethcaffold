...
class {$action_class} extends {$project_id}_ActionClass
{
	...
	function perform()
	{
		...
		$r =& ${$model}Manager->update(${$model});
		if (Ethna::isError($r) || $r === false) {
			$this->ae->add(null, SCAFFOLD_MODEL_NAME_{$modelUName} . "の編集に失敗しました", E_{$modelUName}_UPDATE_FAILED);
			return '{$view_error_name}';
		}
		
		// Notice Message Pattern
		$_notices = array();
		$_notices[] = SCAFFOLD_MODEL_NAME_{$modelUName} . "の編集が完了しました";
